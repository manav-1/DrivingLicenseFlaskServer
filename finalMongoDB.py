...
db = myclient["Manav"]
Collection = db["DrivingLicense"]

def updatetodatabase(image):
    # with open(image, "rb") as image_file:
    #     b64String = base64.b64encode(image_file.read(), encoding="utf-8")
    # print(image.shape)
    # image= image.tolist()
    b64String = str(base64.b64encode(image))
    imagedict["image"] = b64String
    with open("DrivingLicense.json", "r+") as file:
        data = json.load(file)
        data.update(imagedict)
        file.seek(0)
        json.dump(data,file)
